...
// In this package, semantic version strings may begin with a leading "v",
...
// Minor returns the minor version prefix of the semantic version v.
// For example, Minor("v2.1.0") == "1".
...
// For example, MajorMinor("v2.1.0") == "2.1".
...
// Patch returns the patch version prefix of the semantic version v.
// For example, Patch("v3.2.1") == "1".
...
// SortMax returns the versions in order of their version (descending).
// It skips (ignores) invalid semversions.
func SortMax(versions []string) []string {
	...
	sort.Slice(output, func(i, j int) bool {
		return Compare(output[i], output[j]) > 0
	})
	return output
}

// SortMin returns the versions in order of their version (ascending).
// It skips (ignores) invalid semversions.
